<?php
if ( ! class_exists( 'TG_Widget_Home_Latest_Products' ) ) :

	class TG_Widget_Home_Latest_Products extends WP_Widget {
		...
		public function __construct() {
			$widget_ops  = array( 'description' => __( 'Homepage widget. Displays a number of the latest (or random) products, optionally from a specific category.', 'booth-woo' ) );
			$control_ops = array();
			parent::__construct( 'tg-home-latest-products', esc_html__( 'Theme (home) - Latest Products', 'booth-woo' ), $widget_ops, $control_ops );
		}
		...
		public function widget( $args, $instance ) {
			...
			if ( $title || $subtitle ) {
				...
				if ( $title ) {
					echo wp_kses(
						$args['before_title'] . $title . ' <a href="' . esc_url( $more_url ) . '">' . esc_html__( 'See More', 'booth-woo' ) . '</a>' . $args['after_title'],
						booth_woo_get_allowed_sidebar_wrappers()
					);
				}
		...
		public function form( $instance ) {
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'term_id' ) ); ?>"><?php esc_html_e( 'Category to display the latest products from (optional):', 'booth-woo' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>"><?php esc_html_e( 'Order by:', 'booth-woo' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'columns' ) ); ?>"><?php esc_html_e( 'Output Columns:', 'booth-woo' ); ?></label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'columns' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'columns' ) ); ?>" class="widefat">
					<?php
						$col_options = booth_woo_post_type_listing_get_valid_columns_options();
						foreach ( $col_options['range'] as $col ) {
							echo sprintf( '<option value="%s" %s>%s</option>',
								esc_attr( $col ),
								selected( $columns, $col, false ),
								/* translators: %d is a number of columns. */
								esc_html( sprintf( _n( '%d Column', '%d Columns', $col, 'booth-woo' ), $col ) )
							);
						}
					?>
				</select>
		...
		protected function product_orderby() {
			return apply_filters( 'booth_woo_wc_latest_products_orderby', array(
				'date'       => esc_html__( 'Latest', 'booth-woo' ),
				'rating'     => esc_html__( 'Rating', 'booth-woo' ),
				'popularity' => esc_html__( 'Popularity', 'booth-woo' ),
				'rand'       => esc_html__( 'Random', 'booth-woo' ),
			) );
		}
